...
class CustomRGBImgWrapper(RGBImgPartialObsWrapper):
    """
    A wrapper that modifies the observation to include only the RGB image.
    """
    ...
    def observation(self, observation):
        ...
        return {
            "direction": np.array(observation["direction"], dtype=np.float32),
            "image": np.array(observation["image"], dtype=np.float32),
            "position": np.array(self.unwrapped.agent_pos, dtype=np.float32),
        }
